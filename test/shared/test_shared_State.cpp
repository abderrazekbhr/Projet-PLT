...
BOOST_AUTO_TEST_CASE(TestStateInitialization)
{
    ...
    state.setNbPlayer(4);
    BOOST_CHECK_EQUAL(state.incrementTurn(), 0); // Vérifie que le premier tour commence correctement.
...
BOOST_AUTO_TEST_CASE(TestStateTurnManagement)
{
    ...
    BOOST_CHECK_EQUAL(state.incrementTurn(), 1); // Passe au joueur 2.
    BOOST_CHECK_EQUAL(state.incrementTurn(), 2); // Passe au joueur 3.
    BOOST_CHECK_EQUAL(state.incrementTurn(), 3);
    BOOST_CHECK_EQUAL(state.incrementTurn(), 0); // Revient au joueur 1 (tour cyclique).
}
